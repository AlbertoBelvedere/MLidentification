...
parser.add_argument(
   '--ntrees', default=447, type=int               # default: 100. For UL training: 1000
)

parser.add_argument(
   '--depth', default=11, type=int                  # default: 6. For UL training: 10
)
parser.add_argument(
...
#   data = data[data.gsf_mode_pt>1]
   train, validation = train_test_split(data, 10, 8)
...
if args.what in ['seeding', 'fullseeding']:
   eff = float((validation.baseline
 & validation.is_e).sum())/validation.is_e.sum()
